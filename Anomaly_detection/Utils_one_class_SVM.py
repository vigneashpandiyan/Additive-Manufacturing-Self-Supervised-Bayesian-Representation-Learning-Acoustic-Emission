...
def anomaly_detection(dataset,path):
    
    plt.rcParams.update(plt.rcParamsDefault)
    filename='Clustering_'+str(dataset)
    
    train_embeddings = path+'/'+str(filename)+'/'+str(filename)+'_test_embeddings'+ '.npy'
    train_labelsname = path+'/'+str(filename)+'/'+str(filename)+'_test_labels'+'.npy'
    
    X=np.load(train_embeddings)
    y=np.load(train_labelsname)

    X_Features,X_labels,Y_Features,Y_labels = anomaly_data_prep(X, y)
    filename='Anomaly_'+str(dataset)
    anomaly_one_class_SVM(X_Features,X_labels,Y_Features,Y_labels,filename)


def anomaly_one_class_SVM(X_Features,X_labels,Y_Features,Y_labels,filename):
    
    folder = os.path.join('Figures/', filename)
    ...
    plotname=folder+'_anomaly_classification_accuracy.png'
